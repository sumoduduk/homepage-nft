...
const blockExpLink = (address, blockExplorer) =>
  `${blockExplorer || 'https://https://goerli.etherscan.io/'}address/${address}`

const Address = props => {
  const { hasCopied, onCopy } = useClipboard
  const address = props.value || props.address
  const etherscanLink = blockExpLink(address, props.blockExplorer)
  ...
  return (
    <span>
      <span className="flex items-center pl-5">
        ...
        <CopyIcon as="button" onClick={onCopy} ml={2}>
          {hasCopied && (
            <div
